      ...
      <el-col :span="12" :offset="6">
        <el-form label-width="100px">
          ...
          <el-form-item :label="$t('shopName')">
            <el-input v-model="shop_form.name_cn" :maxlength="50" show-word-limit></el-input>
          </el-form-item>
          <el-form-item :label="$t('shopIntroduction')">
            <el-input v-model="shop_form.remark" type="textarea" :maxlength="500" show-word-limit>
            </el-input>
          </el-form-item>
          <label
            style="font-size: 16px;
          ...
          <el-form-item :label="$t('Contactformrmation')">
            <el-input v-model="shop_form.contact.phone" :maxlength="20" show-word-limit></el-input>
          </el-form-item>
          ...
          <el-form-item>
            <el-row>
              <el-col :span="2" :offset="10">
                <el-button :loading="isButtonLoading" type="primary" @click.native="onSubmit">
                  {{ $t('submit') }}
                </el-button>
              </el-col>
            </el-row>
...
export default {
  ...
  methods: {
    ...
    addShop() {
      ...
      $http.addNewShop({ warehouse_id: this.warehouseId, ...this.shop_form }).then(res => {
    ...
    editShop(shopId) {
      ...
      this.shop_form.contact.city = this.shop_form.contact.pre_address[1];
      this.shop_form.contact.district = this.shop_form.contact.pre_address[2];
      $http.editShop(shopId, { warehouse_id: this.warehouseId, ...this.shop_form }).then(res => {
        if (res.status) return;
    ...
    editInit(rowData) {
      ...
      this.shop_form.contact.pre_address = [
        `${rowData.sender_address.province}`,
        `${rowData.sender_address.city}`,
        `${rowData.sender_address.district}`
      ];
    }
  }
};
