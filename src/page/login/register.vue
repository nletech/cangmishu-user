  ...
  <page-model :title="$t('FistRegister')" :type="tips">
    <el-tabs stretch v-model="activeName" style="position: relative; top: -40px;">
      <el-tab-pane :label="$t('EmailRegister')" name="email">
        <el-form ref="registerEmail" :model="form" :rules="rules" :class="$style.input_item">
          <el-form-item :class="$style.item_email">
            <el-input
          ...
          <el-form-item prop="code" :class="$style.item_verificationCode">
            <el-input v-model="form.code" type="text" :placeholder="$t('inputVcode')" size="small">
            </el-input>
          </el-form-item>
          <el-form-item :class="$style.item_password" prop="password">
            <el-input
          ...
          <el-form-item :class="$style.item_warehouseName">
            <el-input
          ...
          <el-form-item>
            <el-input v-model="form.warehouse_area" :placeholder="$t('WarehouseArea')" size="small">
          ...
          <el-form-item :class="$style.save_user_info">
            <div :class="$style.item_user_info">
              <el-checkbox v-model="keep">
                <p>
                  <span>{{ $t('IAgree') }}</span>
                  <span @click="user_agreement_dialog" :class="$style.user_agreement">{{
                    $t('theUserAgreement')
                  }}</span>
                </p>
              </el-checkbox>
          ...
          <el-form-item class="user_login_button">
            <el-button @click="goRegister('email')" type="primary">
              {{ $t('FistRegister') }}
            </el-button>
          </el-form-item>
          <el-button
      ...
      <el-tab-pane :label="$t('MobileReister')" name="phone">
        <el-form ref="register" :model="formPhone" :rules="rules" :class="$style.input_item">
          <el-form-item :class="$style.item_phone">
            <el-input
          ...
          <div :class="$style.item_picture" :style="style.OperateStylePhonePicture">
            <div :class="$style.item_picture_main">
              ...
              <div :class="$style.item_picture_input">
                <el-input
                  v-model="verifyPhone.captcha"
                  size="small"
                  style="width: 230px; position: relative; left: 10px;"
                >
                  <template slot="append">
                ...
                <img
                  style=" width:100px; height: 30px; position: absolute; top: 46px; left: 250px;"
                  :src="vPicture"
                />
              </div>
          ...
          <el-form-item class="user_login_button">
            <el-button @click="goRegister('phone')" type="primary">
              {{ $t('FistRegister') }}
            </el-button>
          </el-form-item>
          <el-button
    ...
    >
      <p style="text-align: left;">
        ...
        &nbsp;&nbsp;1.2
        用户在注册之前，应当仔细阅读本协议，并同意遵守本协议后方可成为注册用户。一旦注册成功，则用户与仓秘书之间自动形成协议关系，用户应当受本协议的约束。用户在使用特殊的服务或产品时，应当同意接受相关协议后方能使用。
        <br />
        &nbsp;&nbsp;1.3
        本协议则可由仓秘书随时更新，用户应当及时关注并同意本站不承担通知义务。本站的通知、公告、声明或其它类似内容是本协议的一部分。<br />
        二、服务内容<br />
        &nbsp;&nbsp;2.1 仓秘书的具体内容由本站根据实际情况提供。<br />
        &nbsp;&nbsp;2.2
        本站仅提供相关的网络服务，除此之外与相关网络服务有关的设备(如个人电脑、手机、及其他与接入互联网或移动网有关的装置)及所需的费用(如为接入互联网而支付的电话费及上网费、为使用移动网而支付的手机费)均应由用户自行负担。<br />
        三、用户帐号<br />
        &nbsp;&nbsp;3.1
        经本站注册系统完成注册程序并通过身份认证的用户即成为正式用户，可以获得本站规定用户所应享有的一切权限。
        <br />
        &nbsp;&nbsp;3.2
        用户需按照根据真实内容注册。用户有义务保证密码和帐号的安全，用户利用该密码和帐号所进行的一切活动引起的任何损失或损害，由用户自行承担全部责任，本站不承担任何责任。如用户发现帐号遭到未授权的使用或发生其他任何安全问题，应立即修改帐号密码并妥善保管，如有必要，请通知本站。
        因黑客行为或用户的保管疏忽导致帐号非法使用，本站不承担任何责任。<br />
        四、使用规则<br />
        &nbsp;&nbsp;4.1
        遵守中华人民共和国相关法律法规，包括但不限于《中华人民共和国计算机信息系统安全保护条例》、《计算机软件保护条例》、《最高人民法院关于审理涉及计算机网络著作权纠纷案件适用法律若干问题的解释(法释[2004]1号)》、
        《全国人大常委会关于维护互联网安全的决定》、《互联网电子公告服务管理规定》、《互联网新闻信息服务管理规定》、《互联网著作权行政保护办法》和《信息网络传播权保护条例》等有关计算机互联网规定和知识产权的法律和法规、实施办法。
        <br />
        &nbsp;&nbsp;4.2
        ...
        &nbsp;&nbsp;4.3
        用户承诺对其发表或者上传于本站的所有信息(即属于《中华人民共和国著作权法》规定的作品，包括但不限于文字、图片、音乐、电影、表演和录音录像制品和电脑程序等)均享有完整的知识产权，或者已经得到相关权利人的合法授权；
        如用户违反本条规定造成本站被第三人索赔的，用户应全额补偿本站一切费用(包括但不限于各种赔偿费、诉讼代理费及为此支出的其它合理费用)；
        <br />
        &nbsp;&nbsp;4.4
        ...
        &nbsp;&nbsp;4.5
        如用户在使用网络服务时违反上述任何规定，本站有权要求用户改正或直接采取一切必要的措施(包括但不限于删除用户张贴的内容、暂停或终止用户使用网络服务的权利)以减轻用户不当行为而造成的影响。<br />
        五、隐私保护<br />
        5.1
        ...
        &nbsp;&nbsp;&nbsp;&nbsp;(4)为维护社会公众的利益。<br />
        &nbsp;&nbsp;5.2
        本站可能会与第三方合作向用户提供相关的网络服务，在此情况下，如该第三方同意承担与本站同等的保护用户隐私的责任，则本站有权将用户的注册资料等提供给该第三方。<br />
        &nbsp;&nbsp;5.3
        在不透露单个用户隐私资料的前提下，本站有权对整个用户数据库进行分析并对用户数据库进行商业上的利用。<br />
        六、版权声明<br />
        &nbsp;&nbsp;6.1
        本站的文字、图片、音频、视频等版权均归湖南恩尔伊网络科技有限公司享有或与作者共同享有，未经本站许可，不得任意转载。
        <br />
        &nbsp;&nbsp;6.2
        本站特有的标识、版面设计、编排方式等版权均属湖南恩尔伊网络科技有限公司享有，未经本站许可，不得任意复制或转载。
        <br />
        &nbsp;&nbsp;6.3
        使用本站的任何内容均应注明“来源于仓秘书”及署上作者姓名，按法律规定需要支付稿酬的，应当通知本站及作者及支付稿酬，并独立承担一切法律责任。<br />
        &nbsp;&nbsp;6.4
        本站享有所有作品用于其它用途的优先权，包括但不限于网站、电子杂志、平面出版等，但在使用前会通知作者，并按同行业的标准支付稿酬。<br />
        &nbsp;&nbsp;6.5
        本站所有内容仅代表作者自己的立场和观点，与本站无关，由作者本人承担一切法律责任。 <br />
        &nbsp;&nbsp;6.6 恶意转载本站内容的，本站保留将其诉诸法律的权利。<br />
        七、责任声明<br />
        &nbsp;&nbsp;7.1
        用户明确同意其使用本站网络服务所存在的风险及一切后果将完全由用户本人承担，仓秘书对此不承担任何责任。
        <br />
        &nbsp;&nbsp;7.2
        本站无法保证网络服务一定能满足用户的要求，也不保证网络服务的及时性、安全性、准确性。 <br />
        &nbsp;&nbsp;7.3
        ...
        &nbsp;&nbsp;7.5
        对于站向用户提供的下列产品或者服务的质量缺陷本身及其引发的任何损失，本站无需承担任何责任：<br />
        &nbsp;&nbsp;&nbsp;&nbsp;(1)本站向用户免费提供的各项网络服务；<br />
        &nbsp;&nbsp;&nbsp;&nbsp;(2)本站向用户赠送的任何产品或者服务。<br />
        &nbsp;&nbsp;7.6
        本站有权于任何时间暂时或永久修改或终止本服务(或其任何部分)，而无论其通知与否，本站对用户和任何第三人均无需承担任何责任。<br />
        八、附则<br />
...
export default {
  ...
  data() {
    ...
    return {
      rules: {
        code: [{ validator: vCode, required: true, tigger: 'bulr' }],
        password: [{ validator: vPassword, required: true, tigger: 'bulr' }],
        warehouse_name: [{ validator: vWarehouseName, required: true, trigger: 'blur' }],
        warehouse_area: [{ validator: vWarehouseArea, required: true, trigger: 'blur' }]
      },
      tips: false,
      showInfo: false,
  ...
  methods: {
    handlerGetPhoneCode() {
      if (this.checkInputPhone(this.formPhone.mobile)) {
        ...
        $http.getVerificationPhoneCode(info).then(res => {
    ...
    handlerGetEmailCode() {
      if (this.checkInputEmail(this.form.email)) {
        ...
        $http.getVerificationEmailCode(info).then(res => {
          if (res.status) return;
          this.$message({
            type: 'success',
            message: this.$t('Sendsuccessfullypleasecheck')
      ...
      if (flag === 'phone') {
        this.$refs.register.validate(valid => {
          if (valid) {
            ...
            $http.register(this.formPhone).then(() => {
              this.$message({
                type: 'success',
