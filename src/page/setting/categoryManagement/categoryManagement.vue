    ...
    <el-dialog :title="getDailogTitle()" :visible.sync="dialogVisible" width="800">
      <el-form ref="editForm" :rules="rules" :model="model" label-width="140px">
        ...
        <el-form-item :label="$t('Expirydate')" prop="need_expiration_date">
          <el-row>
            <el-switch
              v-model="model.need_expiration_date"
              :active-value="1"
              :inactive-value="0"
            ></el-switch>
            <el-popover
        ...
        <el-form-item :label="$t('bestUseTime')" prop="need_best_before_date">
          <el-switch
            v-model="model.need_best_before_date"
            :active-value="1"
            :inactive-value="0"
          ></el-switch>
          <el-popover
...
export default {
    ...
    return {
      ...
      rules: {
        name_cn: [
          {
            required: true,
            message: this.$t('Pleaseentercategoryname'),
            trigger: 'blur'
          }
        ],
        name_en: [
  ...
  methods: {
    ...
    delCategory(row) {
      $http.deleteCategoryManagement(row.id).then(re => {
        if (re.status) return;
    ...
    subimtData() {
      this.$refs.editForm.validate(validate => {
        ...
        if (formData.id > 0) {
          // 修改信息
          $http.editCategoryManagement(formData.id, formData).then(re => {
            if (re.status) return;
            this.$message({
              type: 'success',
          ...
          // 添加信息
          $http.addCategoryManagement(formData).then(re => {
            if (re.status) return;
            this.$message({
              type: 'success',
