<template>
  ...
  *
  ...
  <el-dialog
    ...
    width="60%"
  >
    <el-form :model="form" ref="submitForm" :rules="valid_rules" label-width="120px">
      <el-form-item label="商品名称" prop="product_name">
        {{ form.product_name }}
      </el-form-item>
      <el-form-item label="SKU" prop="relevance_code">
        {{ form.relevance_code }}
      </el-form-item>
      <el-form-item :label="$t('Inboundbatchnumber')" prop="sku">
        {{ form.sku }}
      </el-form-item>
      <el-form-item label="货位库存" prop="stock_num">
        <el-input v-model="form.stock_num" size="small"> </el-input>
      </el-form-item>
      <el-form-item :label="$t('Rack')" prop="location_code">
        <el-input v-model="form.location_code" size="small"> </el-input>
      </el-form-item>
      <el-form-item label="EAN码" prop="ean">
        <el-input v-model="form.ean" size="small"> </el-input>
      </el-form-item>
      <el-form-item
        label="生产批次号"
      ...
      >
        <el-input
          v-model="form.production_batch_number"
          placeholder="请输入生产批次号"
          size="small"
        >
        </el-input>
      </el-form-item>
      <el-form-item label="保质期" v-if="row_data.need_expiration_date" prop="expiration_date">
        <el-date-picker
...
export default {
  ...
  data() {
    return {
      valid_rules: {
        stock_num: [{ required: true, message: '货位库存不能为空', trigger: 'blur' }],
        location_code: [{ required: true, message: '货位不能为空', trigger: 'blur' }],
        ean: [{ required: true, message: 'ean不能为空', trigger: 'blur' }],
        production_batch_number: [
          { required: true, message: '生产批次号不能为空', trigger: 'blur' }
        ],
        expiration_date: [{ required: true, message: '保质期不能为空', trigger: 'blur' }],
        best_before_date: [{ required: true, message: '最佳食用期不能为空', trigger: 'blur' }]
      },
      form: {
        product_name: '', // 商品名称
  ...
  methods: {
    handleComfirm() {
      this.$refs.submitForm.validate(valid => {
        ...
        $http.editInventory(this.row_data.stock_id, this.form).then(res => {
