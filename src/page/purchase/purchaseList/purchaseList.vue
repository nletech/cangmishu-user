        ...
        >
          <el-table-column type="expand">
            <template slot-scope="prop">
              <el-table :data="prop.row.items" border>
                <el-table-column
                  label="#"
                  type="index"
                  width="40"
                ...
                >
                  <template slot-scope="scope">
                    <el-input :disabled="scope.row.status === 3" v-model="scope.row.confirm_num">
                    </el-input>
                  </template>
                </el-table-column>
            ...
            <template slot-scope="scope">
              ...
              <el-tooltip :content="$t('Complete')" placement="top">
                <el-button
                  ...
                  icon="el-icon-sell"
                  v-if="scope.row.status !== 3"
                  @click="finishedPurchase(scope.row)"
                  round
                  :loading="isButtonLoading"
                >
                </el-button>
    ...
    >
      ...
      <el-row style="margin: 30px 0 0 0;">
        <el-col :span="2" :offset="11">
          <el-button type="primary" :disabled="isDisabled" @click="handlerClick">{{
            $t('download')
          }}</el-button>
        </el-col>
      </el-row>
...
export default {
  ...
  methods: {
    handlerClick() {
      ...
      window.open(
        `${baseApi.BASE_URL}purchase/${this.cache.id}/download?api_token=${this.api}&lang`
      );
    },
    ...
    viewDetails(row) {
      ...
      this.$nextTick(() => {
        ...
        const iframe = createAnChildNode('iframe', {
          ...
          height: '500',
          frameBorder: '0',
          src: `${baseApi.BASE_URL}purchase/${row.id}/pdf?api_token=${this.api}`
        });
