  ...
  <model-form>
    <el-form slot="left"
      ...
      <el-form-item :label="$t('cargoProperty')" prop="warehouse_feature_id">
        <el-select v-model="form.warehouse_feature_id">
          <el-option
      ...
      <el-form-item :label="$t('SkuProperty')">
        <el-checkbox v-model="form.need_expiration_date">保质期</el-checkbox>
        <el-checkbox v-model="form.need_production_batch_number">生产批次号</el-checkbox>
        <el-checkbox v-model="form.need_best_before_date">最佳食用期</el-checkbox>
      </el-form-item>
  ...
  methods: {
    getList() {
      $http.warehouseFeature(this.params).then((res) => {
        this.cargoPropertyList = res.data.data.filter(item => !!item.is_enabled);
      });
    },
    getInfo() {
      if (!this.$route.query.categoryId) return;
      $http.getInfoCategoryType(
        this.$route.query.categoryId, { warehouse_id: this.$route.query.id },
      ).then((res) => {
        this.form = res.data;
        this.form.is_enabled = !!res.data.is_enabled;
        this.form.need_production_batch_number = !!res.data.need_production_batch_number;
        this.form.need_expiration_date = !!res.data.need_expiration_date;
        this.form.need_best_before_date = !!res.data.need_best_before_date;
      });
    },
    submitCategoryAdd(formName) {
      // 将布尔值转化为数值
      const formatObj = {};
      formatObj.need_production_batch_number = +this.form.need_production_batch_number;
      formatObj.need_expiration_date = +this.form.need_expiration_date;
      formatObj.need_best_before_date = +this.form.need_best_before_date;
      formatObj.is_enabled = +this.form.is_enabled;
      const senderData = Object.assign({}, this.form, formatObj);
      this.$refs[formName].validate((valid) => {
        if (valid) {
          $http.addCategoryType(senderData, this.$route.query.categoryId).then(() => {
            this.successTips(this.$route.query.categoryId);
          });
        } else {
          this.$message({
            message: '请检查您的输入!',
