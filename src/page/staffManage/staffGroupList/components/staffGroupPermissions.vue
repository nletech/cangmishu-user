  ...
  *
  ...
  <div>
    <el-row :class="$style.Permissions_main">
      <el-col :span="20">
        <el-form label-width="200" :model="permission_form" :class="$style.form_main">
          <el-row :gutter="30">
            <el-col :span="10">
              <!-- 请选择仓库 -->
              <el-form-item :label="is_init ? '当前仓库:' : '请选择仓库:'" prop="warehouse">
                <el-radio-group v-model="form.warehouse_id" @change="handleChange"
                  >yarndev
                  <el-radio
                    :disabled="is_init"
            ...
            <el-col :span="10" :offset="4">
              ...
              <el-form-item label="请选择操作权限:" prop="modules">
                ...
                <el-checkbox-group v-model="checkedArray" @change="handleCheckedChange">
                  <el-checkbox v-for="item in modules" :label="item.id" :key="item.name">
                    {{ item.name }}
                  </el-checkbox>
                </el-checkbox-group>
              </el-form-item>
        ...
        <el-row>
          <el-col>
            <!-- 保存 -->
            <el-button :class="$style.submit_form_icon" size="great" round @click="submit_form">
              保存
            </el-button>
          </el-col>
        </el-row>
...
export default {
  ...
  computed: {
    checked() {
      return {
        warehouse_id: this.temp_warehouse_id,
  ...
  methods: {
    ...
    getData() {
      $http.getStaffDetail(this.$route.params.groupId).then(res => {
        const data = res.data;
        // 总模块
        ...
        if (res.data.group.warehouse_id && GROUP_MODULES.length > 0) {
          // 已选择仓库的情况
    ...
    submit_form() {
      ...
      this.$confirm('确认修改?', '提示', {
        ...
        // 推送修改信息
        $http.addStaffGroupDetail(this.form).then(res => {
          // 处理添加成功
          if (res.status === 0) {
            // 显示成功消息
            this.$message({
