                          ...
                          <article>
                              <article style="width: 80%;">
                                <div>
                                  <p>第1章 引言 </p>
                                  <p><b>1.1 文档用途</b>
                                  <p>本文档描述了仓秘书数据服务开放平台接口，包括接口规范、具体接口的接口编码、 接口名称、功能描述、输入与输出参数定义，以供合作机构接入时使用。</p>
                                  <p><b>1.2 阅读对象 </b></p>
                                  ...
                                  <p><b>第2章 总体设计</b> </p>
                                  <p><b>2.1 接口形式</b></p>
                                  <p>接口协议使用 HTTPS，由服务提供方提供访问的 URL 规范，合作机构使用 HTTPS POST/GET 方法发送请求并获取应答报文，合作机构作为服务提供方的 HTTPS 客户端，服 务提供方作为合作机构的 HTTPS 服务端，发起和返回报文统一使用 UTF-8 编码。外部调用 方与开放平台之间交互同步等待超时时间建议最大值设置为 10 秒。</p>
                                  <p>本文档包含订单新增、查询接口、库存新增查询接口。同时也支持库存变化、订单变化自动推送给合作机构。</p>
                                  ...
                                  <p><a href="https://api.cangmishu.com/open/api/stock/spec?app_key=K0*5&amp;timestamp=1567081963&amp;code=KG1Z%2CKG2Z&amp;sign=e2983dcd*****b5cad535d3**6020" target="_blank">https://api.cangmishu.com/open/api/stock/spec?app_key=K0*5&times;tamp=1567081963&amp;code=KG1Z%2CKG2Z&amp;sign=e2983dcd*****b5cad535d3**6020</a></p>
                                  ...
                                  <p>调用接口时需要对请求包头参数进行签名验证，服务器也会对该请求参数进行验证是否 合法的。过程如下:根据包头参数名称将所有请求参数按照字母先后顺序排序，如:将入参 foo=1, bar=2, baz=3 排序为 bar=2,baz=3,foo=1;再将参数名和参数值连接后，得到拼装后的 字符串 bar2baz3foo1。</p>
                                  <p>系统支持 MD5 加密方式:</p>
                                  ...
                                  <p><b>2.3.5 注意事项</b></p>
                                  <p>所有的请求和响应数据编码皆为 UTF-8 格式。查询类接口建议用 GET 请求，交易等隐</p>
                      ...
                      <el-collapse-item title="推送签名方式简介" name="2">
                          <article>
                              <article style="width: 80%;">
                                ...
                                <p>第1章 引言 </p>
                                <p><b>1.1 文档用途</b>
                                <p>本文档描述了仓秘书数据服务开放平台推送给第三方接口，包括接口规范、具体接口的接口编码、 接口名称、功能描述、输入与输出参数定义，以供合作机构接入时使用。</p>
                                <p> <b>1.2 阅读对象 </b></p>
                                <p>本说明书的读者对象为软件设计人员、软件开发人员、软件测试人员等。</p>
                                <p> <b>1.3 缩略语与名词解释</b> </p>
                                <table style="width: 90%;">
                                ...
                                <p><br /></p>
                                <p><b>2.3.2 调用报文样例</b></p>
                                <p>http(s)://第三方回调网址<a href="https://api.cangmishu.com/open/api/stock/spec?app_key=K0*5&amp;timestamp=1567081963&amp;code=KG1Z%2CKG2Z&amp;sign=e2983dcd*****b5cad535d3**6020" target="_blank">?app_key=K0*5&times;tamp=1567081963&amp;method=stockChange&amp;sign=e2983dcd*****b5cad535d3**6020</a></p>
                                <p><b>2.3.3 返回报文结构</b></p>
                                <table style="width: 90%;">
                                  <tbody>
                                  <tr>
                                ...
                                <p><b>2.3.4 签名 SIGN 机制</b></p>
                                <p>验证调用接口时需要对请求包头参数进行签名验证，服务器也会对该请求参数进行验证是否 合法的。过程如下:根据包头参数名称将所有请求参数按照字母先后顺序排序，如:将入参 foo=1, bar=2, baz=3 排序为 bar=2,baz=3,foo=1;再将参数名和参数值连接后，得到拼装后的 字符串 bar2baz3foo1。</p>
                                <p>系统支持 MD5 加密方式:</p>
                                <p>MD5:将 app_secret 拼接到参数字符串头尾进行 MD5 加密，格式为:</p>
                                <p>MD5(app_secretkey1=value1&amp;key2=value2...)</p>
                                <p><b>2.3.4.1 生成签名步骤</b></p>
                                ...
                                <p>私信息查询和修改类接口建议用 POST 请求。</p>
                                <p><b>第3章 推送接口</b></p>
                                <p>根据合作机构提供的 URL 进行推送，如推送失败会立即重试;如仍然失败则每隔 30 秒重试，最多重试 3 次</p>
                                <p><b>3.1 库存变动</b></p>
                                ...
                                <p><b>3.2 出库单取消</b></p>
                                <p style="color: rgb(51, 51, 51); font-family:">接口类型： orderCancel 接口描述：订单取消，只能请求一次，只能在仓库未拣货的时候取消，其它状态不支持</p>
                                ...
                                <p><b>3.3 发货成功</b></p>
                                <p style="color: rgb(51, 51, 51); font-family:">接口类型： orderShipped 接口描述：订单发货，当仓库填了发货的快递公司后触发</p>
                                <p style="color: rgb(51, 51, 51); font-family:">业务输出参数</p>
                                <table style="width: 90%;">
                                  <tbody>
                                  <tr>
